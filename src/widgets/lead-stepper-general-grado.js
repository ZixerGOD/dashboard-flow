(function () {
  ...
  function getScriptConfig() {
    ...
    return {
      ...
      programsCsvUrl: clean(dataset.programsCsvUrl || dataset.csvUrl || '/programas-CSV/programas.csv'),
  ...
  function parseCsvRows(text) {
    ...
    for (i = 0; i < text.length; i += 1) {
      ...
      if ((ch === '\n' || ch === '\r') && !inQuotes) {
        ...
        row.push(value);
        value = '';
        if (row.length && row.some(function (cell) { return clean(cell) !== ''; })) {
          rows.push(row);
        }
        row = [];
        continue;
  ...
  async function loadPrograms(csvUrl) {
    var response = await fetch(csvUrl, { method: 'GET', cache: 'no-store' });
    if (!response.ok) {
      throw new Error('No se pudo cargar el listado de programas.');
    }

    var text = await response.text();
    var rows = parseCsvRows(text);
    if (!rows.length) {
      throw new Error('El CSV de programas esta vacio.');
    }

    var headers = rows[0].map(normalizeHeader);
    var programIndex = headers.indexOf('PROGRAMA');
    var titleIndex = headers.indexOf('TITLE');
    if (programIndex < 0) {
      throw new Error('El CSV no contiene la columna PROGRAMA.');
    }
    ...
    for (i = 1; i < rows.length; i += 1) {
      var current = rows[i];
      var program = clean(current[programIndex] || '');
      var title = clean((titleIndex >= 0 ? current[titleIndex] : '') || program);
      if (!program || seen[program]) {
        continue;
      }
      seen[program] = true;
      programs.push({ value: program, label: title || program });
    }

    if (!programs.length) {
      throw new Error('No hay programas validos en el CSV.');
    }

    return programs;
  }

  function fillProgramSelect(selectEl, programs, selectedProgram) {
    if (!selectEl) {
      return;
    }

    selectEl.innerHTML = '<option value="">Selecciona un programa</option>';
    ...
    if (selectedProgram) {
      var hasMatch = programs.some(function (program) {
        return clean(program.value) === clean(selectedProgram);
      });
      selectEl.value = hasMatch ? selectedProgram : '';
      return;
    }
  ...
  function emitLeadSuccess(payload) {
    try {
      window.dispatchEvent(new CustomEvent('dm:lead-success', { detail: payload || {} }));
    } catch (error) {
      // no-op
    }
  }
  ...
  function attachWhatsappBehaviour(root, config, utm) {
    ...
    form.addEventListener('submit', async function (event) {
      ...
      var immutableProgram = clean(data.programa || runtimeDataset.programa || config.programa || 'Programa General');
  ...
  function attachBehaviour(root, config, utm) {
    ...
    var programSelect = form.querySelector('#dm-programa-general');
    ...
    loadPrograms(config.programsCsvUrl)
      ...
      .catch(function (error) {
        if (programSelect) {
          programSelect.innerHTML = '<option value="">No se pudieron cargar programas</option>';
        }
        status.className = 'dm-lead-status error';
        status.textContent = clean(error && error.message) || 'No se pudo cargar el listado de programas.';
      });
